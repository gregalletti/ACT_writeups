...
From `Ghidra` we can notice that we only have 2 main functions: `add_numbers()` and `print_numbers`. The second one is just a regular print, while the first one seems to operate in a more complex way. 

All the input is read by a function `get_int()`, that firstly does a read of 200 bytes and stores the result in a char buffer: we have no overflow, but we see that after this an `atoll()` is executed.
...
Of course the numbers leaked change every time, but from `gdb - info files` we can see that, for example, `0x55b6b0af7234` is part of the .text section. Moreover, this specific address never changes, but may be a particular case.
